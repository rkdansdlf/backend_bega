...
public class RankingPredictionService {
	...
	private RankingPredictionResponseDto convertToResponseDto(RankingPrediction prediction) {
		java.util.List<RankingPredictionResponseDto.TeamRankingDetail> details = new java.util.ArrayList<>();
		...
		for (Object[] row : currentRankings) {
			if (row[0] != null && row[1] != null) {
				currentRankMap.put(String.valueOf(row[1]), ((Number) row[0]).intValue());
			}
		}
		...
		for (Object[] row : lastRankings) {
			if (row[0] != null && row[1] != null) {
				lastRankMap.put(String.valueOf(row[1]), ((Number) row[0]).intValue());
			}
		}
